...
from homeassistant.const import SERVICE_TURN_ON, SERVICE_TURN_OFF, STATE_ON, \
    STATE_OFF, ATTR_ENTITY_ID, SUN_EVENT_SUNRISE, SUN_EVENT_SUNSET
...
class PoolPumpManager:
    ...
    def __init__(self, hass, now):
        ...
        sunrise = get_astral_event_date(self._hass,
                                        SUN_EVENT_SUNRISE,
                                        self._now.date())
    ...
    def _build_parameters(self):
        ...
        if swimming_season.state == STATE_ON:
            run_hours_total = min(
                daylight,
                float(self._hass.states.get(
                    self._hass.data[DOMAIN]
                    [ATTR_RUN_PUMP_IN_SWIMMING_SEASON_ENTITY_ID]).state))
            ...
            run_hours_total = min(
                ...
                float(self._hass.states.get(
                    self._hass.data[DOMAIN]
                    [ATTR_RUN_PUMP_IN_OFF_SEASON_ENTITY_ID]).state))
        ...
        if daylight > run_hours_total:
            ...
            _LOGGER.debug("Breaks maximum is %.1f for offset %s and break %s",
                          breaks_maximum, offset, break_duration)
            if breaks_maximum < break_duration + offset:
                # Re-arrange the breaks.
                offset = breaks_maximum / 2
                break_duration = breaks_maximum / 2
                _LOGGER.debug("Shortened offset to %.1f and break to %.1f",
    ...
    def build_runs(self, run_start_time, durations):
        ...
        if len(remaining_durations) >= 2:
            next_start_time = self._round_to_next_five_minutes(
                current_run.stop_time + timedelta(
                    minutes=remaining_durations.pop(0)))
    ...
    def _round_to_next_five_minutes(now):
        ...
        return dt_util.find_next_time_expression_time(now,
                                                      matching_seconds,
                                                      matching_minutes,
                                                      matching_hours)
            ...
            if switch.state == target_state:
                ...
                if target_state == STATE_ON:
                    ...
                    await self._hass.services.async_call('homeassistant',
                                                         SERVICE_TURN_OFF,
                                                         data)
                _LOGGER.info("Switching pool pump from '%s' to '%s'",
