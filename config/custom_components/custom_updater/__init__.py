...
CONFIG_SCHEMA = vol.Schema(
    {
        DOMAIN: vol.Schema(
            {
                vol.Optional(CONF_TRACK, default=DEFAULT_TRACK): vol.All(
                    cv.ensure_list, [cv.string]
                ),
                vol.Optional(CONF_HIDE_SENSOR, default=False): cv.boolean,
                vol.Optional(CONF_SHOW_INSTALLABLE, default=False): cv.boolean,
                vol.Optional(CONF_CARD_CONFIG_URLS, default=[]): vol.All(
                    cv.ensure_list, [cv.url]
                ),
                vol.Optional(CONF_COMPONENT_CONFIG_URLS, default=[]): vol.All(
                    cv.ensure_list, [cv.url]
                ),
                vol.Optional(CONF_PYTHON_SCRIPT_CONFIG_URLS, default=[]): vol.All(
                    cv.ensure_list, [cv.url]
                ),
            }
        )
...
async def async_setup(hass, config):
    ...
    if "cards" in conf_track:
        card_controller = CustomCards(hass, conf_hide_sensor, conf_card_urls, conf_mode)

        hass.bus.async_listen_once(
            EVENT_HOMEASSISTANT_START, card_controller.extra_init()
        )

        async_track_time_interval(hass, card_controller.force_reload, INTERVAL)
    ...
    if "components" in conf_track:
        ...
        hass.bus.async_listen_once(
            EVENT_HOMEASSISTANT_START, components_controller.extra_init()
        )

        async_track_time_interval(hass, components_controller.cache_versions, INTERVAL)
    ...
    if "python_scripts" in conf_track:
        ...
        hass.bus.async_listen_once(
            EVENT_HOMEASSISTANT_START, python_scripts_controller.extra_init()
        )
...
class CustomCards:
    ...
    def __init__(self, hass, conf_hide_sensor, conf_card_urls, conf_mode):
        """Initialize."""
    ...
    async def cache_versions(self, now=None):
        ...
        self.hass.states.async_set("sensor.custom_card_tracker", state, attributes)
...
class CustomComponents:
    ...
    def __init__(self, hass, conf_hide_sensor, conf_component_urls):
        ...
        from pyupdate.ha_custom.custom_components import CustomComponents as Components
    ...
    async def cache_versions(self, now=None):
        ...
        attributes = information[0]
        attributes["hidden"] = self.hidden
        self.hass.states.async_set("sensor.custom_component_tracker", state, attributes)
...
class CustomPythonScripts:
    ...
    def __init__(self, hass, conf_hide_sensor, conf_python_script_urls):
        ...
        self.ha_conf_dir = str(hass.config.path())
        self.hidden = conf_hide_sensor
        self.pyupdate = PythonScripts(self.ha_conf_dir, conf_python_script_urls)
